...
void FDungeonArchitectThemeEditor::OnNodeDoubleClicked(UEdGraphNode* InNode) {
    // Focus of the nearest spawned actor from this theme node
    if (UEdGraphNode_DungeonActorBase* ActorNode = Cast<UEdGraphNode_DungeonActorBase>(InNode)) {
        if (PreviewViewport.IsValid() && PreviewViewport->GetAdvancedPreview().IsValid() && PreviewViewport->GetViewportClient().IsValid()) {
            ...
            const FVector ViewLocation = PreviewViewport->GetViewportClient()->GetViewLocation();
            UWorld* World = PreviewViewport->GetAdvancedPreview()->GetWorld();
            ...
            if (BestActor) {
                ...
                PreviewViewport->GetViewportClient()->FocusViewportOnBox(Bounds);
            }
        }
    }
...
void FDungeonArchitectThemeEditor::SelectedNodesChanged(const TSet<UObject*>& SelectedNodes) {
...
void FDungeonArchitectThemeEditor::InitDungeonEditor(const EToolkitMode::Type Mode,
    ...
    ThemeGraphHandler->NodeDoubleClicked = FDungeonArchitectThemeGraphHandler::FSingleNodeDelegate::CreateSP(this, &FDungeonArchitectThemeEditor::OnNodeDoubleClicked);
    ThemeGraphHandler->SelectedNodesChanged = FDungeonArchitectThemeGraphHandler::FMultiNodeDelegate::CreateSP(this, &FDungeonArchitectThemeEditor::SelectedNodesChanged);
