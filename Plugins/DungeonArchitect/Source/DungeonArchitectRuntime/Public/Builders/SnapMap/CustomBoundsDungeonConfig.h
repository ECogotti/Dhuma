...
#include "CustomBoundsDungeonConfig.generated.h"

class UCustomBoundsModuleDB;
DECLARE_LOG_CATEGORY_EXTERN(CustomBoundsDungeonConfigLog, Log, All);

UCLASS(Blueprintable)
class DUNGEONARCHITECTRUNTIME_API UCustomBoundsDungeonConfig : public UDungeonConfig
{
	GENERATED_BODY()
public:
   /** A module database asset that contains the references of the modules to use for building the dungeon */
   UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Dungeon)
      UCustomBoundsModuleDB* ModuleDatabase;
